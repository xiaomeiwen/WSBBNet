...
public class EZ_UIButton : AutoSpriteControlBase
{
	/// <summary>
	...
	public virtual void SetControlState(CONTROL_STATE s, bool suppressTransitions)
	{
		...
		if (
			   !(
					...
					(
						(prevTransition == transitions[(int)EZ_UIButton.CONTROL_STATE.ACTIVE].list[0] || prevTransition == transitions[(int)EZ_UIButton.CONTROL_STATE.ACTIVE].list[1])
						&&
		...
		else  // Else, have our desired transition run when the active transition is complete:
		{
			...
			QueueTransition((int)s, (int)EZ_UIButton.CONTROL_STATE.ACTIVE, suppressTransitions);
		}
	}
	...
	static public EZ_UIButton Create(string name, Vector3 pos)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
		return (EZ_UIButton)go.AddComponent(typeof(EZ_UIButton));
	}
	...
	/// <returns>Returns a reference to the component.</returns>
	static public EZ_UIButton Create(string name, Vector3 pos, Quaternion rotation)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
