...
/// This differs from EZ_UIButton in that it has no sprite
...
//			SetState((int)m_ctrlState);
	...
	public virtual void SetControlState(CONTROL_STATE s, bool suppressTransitions)
	{
		...
		if (!(alwaysFinishActiveTransition &&
			(prevTransition == transitions[(int)EZ_UIButton.CONTROL_STATE.ACTIVE].list[0] ||
			 prevTransition == transitions[(int)EZ_UIButton.CONTROL_STATE.ACTIVE].list[1])
			))
		{
		...
		else  // Else, have our desired transition run when the active transition is complete:
		{
			...
			if (suppressTransitions)
				return;
			QueueTransition((int)s, (int)EZ_UIButton.CONTROL_STATE.ACTIVE);
		}
	}
