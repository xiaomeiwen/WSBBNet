...
public class EZ_UIProgressBar : AutoSpriteControlBase
{
	...
	public override void Copy(SpriteRoot s, ControlCopyFlags flags)
	{
		...
		if (!(s is EZ_UIProgressBar))
			return;

		if (Application.isPlaying)
		{
			EZ_UIProgressBar b = (EZ_UIProgressBar)s;
	...
	static public EZ_UIProgressBar Create(string name, Vector3 pos)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
		return (EZ_UIProgressBar)go.AddComponent(typeof(EZ_UIProgressBar));
	}
	...
	/// <returns>Returns a reference to the component.</returns>
	static public EZ_UIProgressBar Create(string name, Vector3 pos, Quaternion rotation)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
