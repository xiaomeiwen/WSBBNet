...
public class EZ_UISlider : AutoSpriteControlBase
{
		...
		if(Application.isPlaying)
		{
			...
			else
			{
				if (manager != null)
				{
					...
					knob.SetControlState(EZ_UIButton.CONTROL_STATE.ACTIVE);	// We have to change to active so that when we set to normal, it isn't considered a redundant change and ignored.
					knob.SetControlState(EZ_UIButton.CONTROL_STATE.NORMAL);
				}
				else
	...
	static public EZ_UISlider Create(string name, Vector3 pos)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
		return (EZ_UISlider)go.AddComponent(typeof(EZ_UISlider));
	}
	...
	/// <returns>Returns a reference to the component.</returns>
	static public EZ_UISlider Create(string name, Vector3 pos, Quaternion rotation)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
